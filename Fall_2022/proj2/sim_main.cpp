...
int main(int argc, char** argv, char** env) {
    ...
    /* writeback stage*/ 
        ...
        std::cout <<" PC_WB: 0x" <<  std::hex <<  (int)dut->pipeline->my_WB_stage->WB_counters[1];
        std::cout <<" Inst_WB: 0x" <<  std::hex <<  (int)dut->pipeline->my_WB_stage->WB_counters[2];
        std::cout <<" Op_I:"  <<  std::dec <<  (int)dut->pipeline->my_WB_stage->WB_counters[6];
         int wr_reg_WB = (int)dut->pipeline->my_WB_stage->WB_counters[3];
        if (wr_reg_WB) {
        std::cout <<" wr_reg_WB:"  <<  std::dec <<  wr_reg_WB; 
        std::cout <<" regval_WB:"  <<  std::dec <<  (int)dut->pipeline->my_WB_stage->WB_counters[4];

        std::cout <<" wregno_WB:"  <<  std::dec <<    (int)dut->pipeline->my_WB_stage->WB_counters[7];
        }
        std::cout << std::endl; 
